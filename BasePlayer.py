        ...
        try:
            ...
            while True:
                ...
                if "Game over" in question:
                    ...
                    break
                print(question)
                input_thread = threading.Thread(target=self.get_player_input)
                input_thread.start()

                # Wait for 10 seconds or until the input is received
                input_thread.join(timeout=10)

                # If the input is not received within 10 seconds, set it to an empty string
